        ...
        <LayoutPage title={`Заявка №${order?.number_order}`}>
            ...
            <ModalWindow trigger={rejectPopup}>
                ...
                <div className="modalWindow__body-buttons">
                    <Button text="Закрыть" click={() => {
                        setRejectPopup(false)
                        setRejectID(null)
                    }} />
                    <Button text="Отправить" type="fill" click={() => {
            ...
            {!order?.answer && !order?.is_canceled && <div className="order__buttons">
                {order?.user?._id === user?._id && order?.is_buy && (
                    <>
                        {getDifferenceBetweenTwoDates(order?.date_buy, new Date()) < 24 && <Button text="Отказаться" click={() => {
                            setRejectPopup(true);
