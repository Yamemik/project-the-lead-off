...
const AdminPanelOrder = () => {
	...
	const getUploads = () => {
		let arr = [];
		order.upload.map(({ path }) =>
			arr.push(sliceBigString(path.split("/")[1])),
		);
		return arr;
	};
	...
	return (
		<LayoutPage title={`Заявка №${order?.number_order}`}>
			...
			<div className="order__buttons">
				<Button
					text="Удалить"
					click={() => setDeleteOrderPopup(!deleteOrderPopup)}
				/>
				<Button
					type="fill"
					text="Редактировать"
					click={() =>
						(window.location.href = `/platform/admin-panel/edit-order/${params.id}`)
					}
				/>
			</div>
