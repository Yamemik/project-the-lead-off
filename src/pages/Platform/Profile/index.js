...
const Profile = () => {
			...
			.then(({ data }) => {
				axios
					...
					.then(res => {
						...
						data.map(item => {
							if (
								item.payment.status !== "buy" &&
								item.payment.status !== "refund"
							) {
								console.log(item);
								arr = [
									...arr,
	...
	const getUserTelephone = () => {
		...
		return `+7 ${telephone.slice(2, 5)} ${telephone.slice(
			5,
			8,
		)} ${telephone.slice(8, 10)} ${telephone.slice(10, 12)}`;
	};
			...
			if (!item.checked) {
				...
				axios
					.post("/api/user/me/ukassa", {
						...
						type: "redirect",
						return_url: "https://lothugrale.beget.app/platform/profile",
		...
		<LayoutPage title="Личный счёт и информация">
			...
			<LayoutBlocks addClass="layoutBlocks__profile">
				...
				<LayoutBlock title="Пополнение счета">
					...
					{Number(JSON.parse(localStorage.getItem("user")).credit) >
						0 && (
						<p className="profile__deposit-attention">
							Внимание! Следующие пополнения пойдут на погашение
							кредита!
						</p>
					)}
				...
				<LayoutBlock title="История списаний">
					...
					{offsItemsOffset < offsItems.length && (
						<Button
							text="Посмотреть ещё"
							click={() => setOffsItemsOffset(prev => prev + 5)}
						/>
					)}
				</LayoutBlock>
				<LayoutBlock title="История пополнений">
					...
					{refillItemsOffset < refillItems.length && (
						<Button
							text="Посмотреть ещё"
							click={() => setRefillItemsOffset(prev => prev + 5)}
						/>
					)}
				</LayoutBlock>
			</LayoutBlocks>
